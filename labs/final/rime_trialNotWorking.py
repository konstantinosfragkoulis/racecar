...
LEFT45_WINDOW = (305, 325) # Degrees not indecies of the array scan
RIGHT45_WINDOW = (35, 55) # Degrees not indecies of the array scan
CENTER0_WINDOW = (350, 10) # Degrees not indecies of the array scan
...
def slalomCones():
    ...
    if hax3 < 10:
        ...
        for i in range(0, CAMERA_WIDTH):
            if centerRow[i][0] > 214 and centerRow[i][1] > 195 and centerRow[i][2] > 175 and depthImg[CAMERA_WIDTHd2][i] < 300:
...
def followWalls():
    ...
    left45Angle, left45Dist = rc_utils.get_lidar_closest_point(scan, LEFT45_WINDOW)
    right45Angle, right45Dist = rc_utils.get_lidar_closest_point(scan, RIGHT45_WINDOW)
    center0Angle, center0Dist = rc_utils.get_lidar_closest_point(scan, CENTER0_WINDOW)

    if lock == 'C':
        leftPoint = left45Dist
        rightPoint = right45Dist
        leftPointAngle = left45Angle
        rightPointAngle = right45Angle
        speed = 1

    if center0Dist < left45Dist or center0Dist < right45Dist:
        if lock == 'C':
            if left45Dist < right45Dist:
                lock = 'R'
                leftPoint = center0Dist
                rightPoint = right45Dist
                leftPointAngle = center0Angle
                rightPointAngle = right45Angle
                speed = 0.5
            else:
                lock = 'L'
                leftPoint = left45Dist
                rightPoint = center0Dist
                leftPointAngle = left45Angle
                rightPointAngle = center0Angle
                speed = 0.5
        elif lock == 'R':
            leftPoint = center0Dist
            rightPoint = right45Dist
    ...
    #angle = rc_utils.remap_range(leftPoint - rightPoint, 50, -50 , -1, 1 , True)
    if leftPointAngle > 180:
        leftPointAngle -= 360
    if rightPointAngle > 180:
        rightPointAngle -= 360
    
    _angle = (leftPointAngle * leftPoint + rightPointAngle * rightPoint) / (leftPoint + rightPoint)
    #if _angle < 0:
    #    _angle += 360
    # 270, 90
    angle = rc_utils.remap_range(_angle, -50, 50, -1, 1, True)

    print("LOCK:", lock)
    print("Left Point:", leftPoint)
    print("Right Point:", rightPoint)
    print("Left Point Angle:", leftPointAngle)
    print("Right Point Angle:", rightPointAngle)
    print("_ANGLE:", _angle)
    print("ANGLE:", angle)

    #if leftPoint > rightPoint:
    #    angle = leftPointAngle
    #else:
    #    angle = rightPointAngle
    #angle = rc_utils.remap_range((leftPointAngle + rightPointAngle) / 2, -90, 90, -1, 1, True)
