...
def update_contour():
    ...
    if image is None:
        ...
        # Find all of the red, green, and blue contours
        ...
        # Select the largest contour fo each color
        contourR = rc_utils.get_largest_contour(contoursR, MIN_CONTOUR_AREA)
        contourG = rc_utils.get_largest_contour(contoursG, MIN_CONTOUR_AREA)
        contourB = rc_utils.get_largest_contour(contoursB, MIN_CONTOUR_AREA)
        
        # Selct the contour which exists with the highest priority
        if contourR is not None:
            contour = contourR
        elif contourG is not None:
            contour = contourG
        elif contourB is not None:
            contour = contourB
        else:
            contour = None
...
def update():
    ...
    if angle > 0.001 and angle + 0.1 <= 1:
        angle += 0.1
    elif angle < -0.001 and angle - 0.1 >= -1:
        angle -= 0.1
