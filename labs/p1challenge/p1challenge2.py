...
def update():
    ...
    if car_mode == Mode.red_align:
        if (
            ...
            counter = REVERSE_BRAKE
            car_mode = Mode.red_reverse
        else:
    ...
    if car_mode == Mode.red_pass:
        angle = rc_utils.remap_range(counter, 1, 0, 0, -0.5)
        speed = PASS_SPEED

        counter -= rc.get_delta_time()
        if counter <= 0:
            car_mode = Mode.blue_align if blue_distance > 0 else Mode.blue_find

    elif car_mode == Mode.blue_pass:
        angle = rc_utils.remap_range(counter, 1, 0, 0, 0.5)
        speed = PASS_SPEED

        counter -= rc.get_delta_time()
        if counter <= 0:
            car_mode = Mode.red_align if red_distance > 0 else Mode.red_find

    elif car_mode == Mode.red_find:
        angle = 1
        speed = SEARCH_SPEED
        if red_distance > 0:
            car_mode = Mode.red_align

    elif car_mode == Mode.blue_find:
        angle = -1
        speed = SEARCH_SPEED
        if blue_distance > 0:
            car_mode = Mode.blue_align

    elif car_mode == Mode.red_reverse:
        if counter >= 0:
            counter -= rc.get_delta_time()
            speed = -1
            angle = 1
    ...
    elif car_mode == Mode.blue_reverse:
        if counter >= 0:
            counter -= rc.get_delta_time()
            speed = -1
            angle = 1
    ...
    print(
        f"Mode: {car_mode.name}, red_distance: {red_distance:.2f} cm, blue_distance: {blue_distance:.2f} cm, speed: {speed:.2f}, angle: {angle:2f}"
    )
