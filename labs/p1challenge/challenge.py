...
def get_contour(HSV, MIN_CONTOUR_AREA = 30):
    ...
    if image is None:
        ...
        return rc_utils.get_largest_contour(rc_utils.find_contours(image, HSV[0], HSV[1]))
...
def update():
    ...
    global curr_state, counter, generic_counter
    ...
    if curr_state == State.search:
        print(f"search {generic_counter}")
        if RECOVER_RED: 
            angle = -0.75
        if RECOVER_BLUE:
            angle = 0.75
        ...
        print(f"approach red {generic_counter}")
        ...
        print(f"approach blue {generic_counter}")
        ...
        print(f"turn red {generic_counter}")
        ...
        print(f"turn blue {generic_counter}")
        counter += rc.get_delta_time()
        if counter < 0.65:
    ...
    generic_counter += rc.get_delta_time()
